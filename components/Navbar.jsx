    ...
    return (
        <div className={` w-full flex justify-between fixed z-50  bg-gradient-to-t from-transparent to-black  h-15 ${navColor} ${styling.mainNav}`} >
            <div className='w-2/12 flex justify-center  ml-4'>
                <Image onClick={()=>{
                    location.assign('/')
                }} className=' cursor-pointer w-[60%] h-15'  src={"/logo.png"} width={100} height={10} ></Image>
            </div>
            ...
            <div className='w-2/12'>
                <div className='w-full pt-3 '>
                    <Link className='text-amber-50 text-2xl' href='/signup'>Sign up</Link>
                    <span className='text-amber-50'> | </span>
                    <Link className='text-amber-50  text-2xl' href={'/signin'}>Sign in</Link>
                </div>
            </div>
        </div>
    );
